...
Antes de comenzar con el desarrollo de la aplicación, es necesario definir qué se espera de la misma y bajo qué condiciones debe funcionar. Para ello, en este capítulo se recogen los requisitos del sistema, separando aquellos relacionados con las funcionalidades que debe ofrecer la aplicación de aquellos que describen restricciones de calidad, rendimiento, seguridad o usabilidad.

Esta especificación sirve como base para justificar las decisiones tomadas durante el diseño y la implementación del proyecto. Además, permite comprobar posteriormente si la aplicación desarrollada cumple con los objetivos planteados inicialmente.
...
El backend utiliza Spring Data JPA y un repositorio \texttt{CrudRepository} para almacenar objetos de tipo \texttt{M3DTO}, clase Data Transfer Object usada para devolver el estado de la máquina con formato JSON. La configuración actual del proyecto utiliza el motor de base de datos relacional H2 en memoria mediante la propiedad  \texttt{spring.datasource.url=jdbc:h2:mem:testdb}. Por tanto, aunque se usa una base de datos, los datos no permanecen tras reiniciar el servidor.
...
La figura \ref{diagrama_er} muestra una representación del modelo entidad-relación de la base de datos. Aunque en el código la clase persistida es \texttt{M3DTO}, algunas de sus propiedades son listas anotadas con \texttt{@ElementCollection}. Por este motivo, JPA genera tablas auxiliares para almacenar los rotores, sus posiciones, sus configuraciones y los cables asociados a cada máquina.

El diagrama puede consultarse en la figura \ref{diagrama_er} del apéndice \ref{apendice_figuras}.
...
En el frontend también existe un estado mantenido en memoria mediante React. Este estado incluye, entre otros elementos, la máquina actualmente mostrada, la letra de salida, el historial de la sesión actual, los últimos pasos consultados, la página activa, el modo de cifrado o descifrado, el cable seleccionado temporalmente, la apertura de menús y paneles, y los valores actuales de accesibilidad.

Esta información se pierde al recargar la página, salvo los datos que también se escriben en \texttt{localStorage}: los tokens de autenticación, el historial persistente asociado al usuario autenticado y las preferencias de accesibilidad.

\subsection{Información guardada en disco}

El backend no guarda información propia de la aplicación en disco durante la ejecución normal. La base de datos H2 es en memoria, por lo que las máquinas creadas desaparecen al reiniciar el servidor.

En el lado del cliente sí existe almacenamiento persistente en el navegador mediante \texttt{localStorage}. A diferencia de \texttt{sessionStorage}, esta información no desaparece al cerrar la pestaña o el navegador, sino que se conserva hasta que la aplicación la elimine, el usuario borre los datos del navegador o sea sobrescrita por nuevos valores. Se guardan los siguientes datos en \texttt{localStorage}:
